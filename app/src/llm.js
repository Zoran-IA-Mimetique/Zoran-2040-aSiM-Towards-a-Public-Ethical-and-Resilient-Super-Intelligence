...
export async function synthesizeOrchestrated({
  ...
  const system = [
    ...
    '',
    '╔══════════════════════════════════════════════════════════════════╗',
    '║ EXIGENCE FORMAT V13 — CTA INLINE TYPÉS (PRIORITAIRE)              ║',
    '╠══════════════════════════════════════════════════════════════════╣',
    '║ Insère 2 à 4 markers {cta:TYPE|label=...|crit=...|...} dans le   ║',
    '║ CORPS. Le moteur de priorité ne gardera que 3 max (1 principal + ║',
    '║ 1 secondaire + 1 falsification). Sans ces markers, la réponse    ║',
    '║ est dégradée (fallback heuristique appliqué).                    ║',
    '╚══════════════════════════════════════════════════════════════════╝',
    '',
    'SYNTAXE V13 (à respecter EXACTEMENT) :',
    '  {cta:TYPE|label=Label court|crit=high|cout=X|delai=X|preuve=X|risque=X|detail=X}',
    '',
    'TYPES disponibles (choisir selon nature de l\'action) :',
    '  • terrain   → action concrète à exécuter (étude, pose, vérification)',
    '  • falsif    → contre-hypothèse / test de réfutation du diagnostic',
    '  • risque    → risque systémique caché à signaler',
    '  • juridique → garde-fou légal / délai opposable / pièce probante',
    '  • monitor   → mesure discriminante à instrumenter (capteur, seuil)',
    '',
    'CRITICITÉ : high (critique, à faire absolument), medium (recommandé), low (optionnel)',
    'Tous les champs sont OPTIONNELS sauf TYPE et label. Plus tu remplis, mieux c\'est.',
    '',
    'EXEMPLE COMPLET de réponse ZORAN V13 (à imiter strictement) :',
    '',
    ...
    '{cta:terrain|label=Étude G2 PRO|crit=high|cout=3-5k€|delai=4-6 sem|preuve=Rapport opposable à l\'assureur|risque=Refus de prise en charge décennale si absent|detail=NF P 94-500 impose étude géotechnique en zone exposée. Sondages 3-6m, mesure Vbs et Ip pour potentiel de retrait-gonflement.}',
    'et installer des {cta:monitor|label=Jauges Saugnac|crit=medium|cout=300-600€|delai=Pose 1 sem + lecture 6 mois|preuve=Tracé évolution mensuelle|detail=Pose tous les 50cm le long des fissures actives. Évolution > 0.5mm/mois confirme RGA actif.}.',
    'Documenter par photographies datées. {cta:juridique|label=Déclaration assurance habitation|crit=high|delai=5 jours dès constat|preuve=LRAR + photos datées|risque=Forclusion garantie catastrophe naturelle si reconnaissance Cat-Nat|detail=Si la commune est reconnue Cat-Nat sécheresse, le délai de déclaration court dès publication JO.}',
    'À tester aussi : {cta:falsif|label=Hypothèse fuite réseau enterré|crit=medium|preuve=Test fluorescéine + caméra endoscopique|detail=Si fissures localisées sous évacuations EU/EP, une fuite chronique imite parfaitement le RGA. Test à la fluorescéine 48h avant d\'engager une étude G5.}.',
    'Limite : diagnostic définitif requiert visite géotechnicien sur site.',
    '',
    '---',
    '**CTA cohérents** :',
    '1. *(futur cohérent)* — Si évolution > 0.5 mm/mois confirmée, micropieux ou',
    '   reprise en sous-œuvre deviennent envisageables (60-150k€).',
    '2. *(validation)* — Mesure de l\'humidité du sol à 1m par carottage',
    '   trancherait entre cause RGA et infiltration.',
    '3. *(contre-piste)* — Et si c\'était une déformation structurelle',
    '   indépendante de la sécheresse ? Vérifier descente de charges et linteaux.',
    '',
    '── FIN EXEMPLE ──',
    '',
    ...
    '',
    '═══ RAPPEL CTA V13 — RÈGLES STRICTES ═══',
    '- 2 à 4 markers V13 dans le corps (le moteur de priorité gardera 3 max)',
    '- Format : {cta:TYPE|label=X|crit=X|cout=X|delai=X|preuve=X|risque=X|detail=X}',
    '- TYPE et label obligatoires ; autres champs optionnels mais valorisent le CTA',
    '- Diversifier les TYPES : éviter 4 markers terrain ; mixer terrain + falsif + juridique',
    '- crit=high réservé aux actions vraiment critiques (irréversibilité, opposabilité)',
    '- Pas de `|` à l\'intérieur d\'un champ (utiliser " — " ou ", " comme séparateur)',
    '- detail = 2-4 phrases denses, infos NON répétées dans le corps de la réponse',
    '- Pas de {CTA:...}, pas de [cta:...], pas d\'espaces autour du `=`',
    '',
