"""Essai PROXY-C-001 — exécution du protocole pré-enregistré.

Le protocole est figé dans `PRE-ENREGISTREMENT-PROXY-C-001.md`. Ce script ne
fait que l'exécuter : toute constante qu'il contient y est déclarée.

    python -m experiments.run_transfer_001
"""
...
from ztemps.proxy import RelationalOverlapProxy
from ztemps.systems import SyntheticRun, dephasing_family, perturbed_memory_family

# --- Constantes pré-enregistrées (§2, §3, §4, §5) -------------------------
TOLERANCE = 0.05
UNIFORM_WEIGHT = 0.25
...
def events_of(run: SyntheticRun) -> list[Transformation]:
    ...
    proxy = RelationalOverlapProxy(spec=run.spec, tolerance=TOLERANCE)
    return [
        Transformation(
            sample_id=f"{run.label}-{index}",
            profile=proxy(run.states[index - 1], run.states[index]),
            object_id=run.label,
            proxy_id=f"RelationalOverlapProxy(θ={TOLERANCE})",
        )
...
def cumulative_measure(
    ...
    for index in range(1, len(events) + 1):
        result = tau_z(
            events[:index], weights, tau_star=1.0, evaluation_sample_ids=ids
        )
...
def main() -> int:
    ...
    for run in (calibration, memory):
        events = events_of(run)
        integration = tau_z(events, weights, tau_star=1.0)
    ...
    out = Path(__file__).with_name("resultats_001.json")
